...
#=====================================================
# 2. CV Augmentation (Voltage Shift + Current Scaling)
#=====================================================
def varry_CV(CV_data):
    ...
    shifted_voltage = voltage + shift_amount

    # Random current scaling (±10%)
    current_scale_factor = np.random.uniform(0.9, 1.1)
    shifted_current = current * current_scale_factor

    # Return modified DataFrame
    new_CV = pd.DataFrame({'Voltage': shifted_voltage,'Current': shifted_current})
...
#====================================
results_dir = os.path.join(base_dir, "results","script1")
